...
def _set_sequence_duration(root: ET.Element, duration_seconds: float) -> None:
    """Keep generated XSQ media duration aligned with the analyzed audio."""
    head = root.find("head")
    if head is None:
    ...
    duration_el.text = f"{max(0.0, float(duration_seconds)):.3f}"
...
def run_variant(
    ...
    _set_sequence_duration(xsq.root, audio.dur_s)
