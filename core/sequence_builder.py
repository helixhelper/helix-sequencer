...
def _normalize_engine_args(engine_args: list[str] | None) -> list[str] | None:
    """Allow `main.py --profile master -- song.wav` as shorthand for `--audio song.wav`."""
    if engine_args is None:
        return None
    ...
    positional_audio = [
        ...
        if (
            ...
            and Path(arg).suffix.lower() in {".wav", ".mp3", ".m4a", ".flac", ".ogg", ".aiff", ".aif"}
        )
    ]
    if not positional_audio:
        return args
    filtered = [arg for arg in args if arg not in positional_audio]
    return ["--audio", *positional_audio, *filtered]
...
def run_profile(profile_id: str | None, engine_args: list[str] | None = None) -> None:
    ...
    raw_engine_args = list(_normalize_engine_args(engine_args) or [])
    ...
    try:
        ...
        try:
            # The BeatGrid wrapper consumes explicit/default snap flags before
            # delegating to the effect engine, so Prime's automatic grid must
            # remain in the effective arguments here.
