...
def test_run_config_from_engine_args_supports_equals_and_aliases(tmp_path: Path) -> None:
    config = RunConfig.from_engine_args(
        "master",
        [
            f"--template={tmp_path / 'template.xsq'}",
            f"--audio-path={tmp_path / 'song.wav'}",
            f"--layout={tmp_path / 'layout.xml'}",
            f"--output-root={tmp_path / 'outputs'}",
            "--variants=3",
            "--controller-padding=60",
            "--disable-orchestrator",
            "--no-promote-orchestrated-template",
        ],
    )

    assert config.template_path == tmp_path / "template.xsq"
    assert config.audio_path == tmp_path / "song.wav"
    assert config.layout_path == tmp_path / "layout.xml"
    assert config.output_root == tmp_path / "outputs"
    assert config.variants == 3
    assert config.controller_padding == 60
    assert config.enable_orchestrator is False
    assert config.promote_orchestrated_template is False


def test_run_config_to_engine_args_round_trips_important_paths(tmp_path: Path) -> None:
    config = RunConfig(
        profile="master",
...
def test_run_config_validate_inputs_catches_required_none_paths() -> None:
    errors = RunConfig().validate_inputs(require_existing=True)

    assert any("audio path is required" in error for error in errors)
    assert any("template path is required" in error for error in errors)
    assert any("layout path is required" in error for error in errors)


@pytest.mark.parametrize("source_name", ["song.wav", "template.xsq", "layout.xml"])
def test_run_config_validate_inputs_catches_dangerous_output_source_overlap(
    tmp_path: Path,
    source_name: str,
) -> None:
...
def test_run_config_validate_inputs_reports_invalid_variants_and_padding(tmp_path: Path) -> None:
    config = RunConfig(output_root=tmp_path / "outputs", variants=0, controller_padding=-1)

    errors = config.validate_inputs(require_existing=False)
...
def test_run_manager_creates_run_files_and_started_manifest(tmp_path: Path) -> None:
    ...
    assert ctx.run_dir.parent == config.output_root / "beta"
    assert ctx.run_dir.name.endswith("-master")
    assert ctx.command_path.exists()
    assert ctx.command_path.read_text(encoding="utf-8") == "python main.py\n"
    assert ctx.manifest_path.exists()
    assert ctx.log_path.exists()
    assert manifest["schema"] == MANIFEST_SCHEMA
    assert manifest["app"] == "Helix Sequencer"
...
def test_run_manager_finalizes_success(tmp_path: Path) -> None:
    config = RunConfig(output_root=tmp_path / "outputs")

    with RunManager(config).start(command="python main.py", require_existing=False) as ctx:
...
def test_run_manager_context_finalizes_success_by_default(tmp_path: Path) -> None:
    config = RunConfig(output_root=tmp_path / "outputs")
...
def test_run_manager_records_artifacts_with_exists_flag(tmp_path: Path) -> None:
    config = RunConfig(output_root=tmp_path / "outputs")

    with RunManager(config).start(command=["python", "main.py"], require_existing=False) as ctx:
        existing = ctx.run_dir / "sequence.xsq"
        missing = ctx.run_dir / "missing.report.json"
        existing.write_text("<xsequence />", encoding="utf-8")
        ctx.record_artifact("xsq", existing)
        ctx.record_artifact("report", missing)

    manifest = json.loads(ctx.manifest_path.read_text(encoding="utf-8"))
    assert manifest["artifacts"] == [
        {"kind": "xsq", "path": str(existing), "exists": True},
        {"kind": "report", "path": str(missing), "exists": False},
    ]


def test_run_manager_does_not_modify_source_files_after_start_or_finalize(tmp_path: Path) -> None:
    template = tmp_path / "template.xsq"
    audio = tmp_path / "song.wav"
    layout = tmp_path / "layout.xml"
    originals = {
        template: b"template source",
        audio: b"audio source",
        layout: b"layout source",
    }
